...
ssh "$target" "id seedhibaat >/dev/null 2>&1 || useradd --system --home /var/lib/seedhibaat --shell /usr/sbin/nologin seedhibaat; install -d -o root -g root -m 0755 /opt/seedhibaat /opt/seedhibaat/bin /opt/seedhibaat/config /opt/seedhibaat/config/workflows; install -d -o seedhibaat -g seedhibaat -m 0700 /var/lib/seedhibaat /var/backups/seedhibaat; install -d -o root -g root -m 0700 /etc/seedhibaat"
