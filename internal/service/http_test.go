...
func TestCustomerImportLinksAPhoneShopifyWillNotReturn(t *testing.T) {
	server, db := testHTTPServer(t)
	defer db.Close()
	server.Config.PIIHashKey = "hash-key-value-for-tests"
	now := time.Now().UTC().Format(time.RFC3339Nano)
	// Imported from Shopify with no phone, because protected customer data is
	// withheld from the app.
	_, _ = db.DB.Exec(`INSERT INTO customers(id,shopify_id,whatsapp_consent,created_at,updated_at) VALUES(30,'gid://shopify/Customer/30','unknown',?,?)`, now, now)

	body := `{"records":[{"shopify_id":"gid://shopify/Customer/30","phone":"+919876500030"}]}`
	request := httptest.NewRequest(http.MethodPost, "/api/v1/customers/import", bytes.NewReader([]byte(body)))
	request.Header.Set("Authorization", "Bearer api")
	response := httptest.NewRecorder()
	server.Handler().ServeHTTP(response, request)
	...
	if err := db.DB.QueryRow(`SELECT phone_hash FROM customers WHERE id=30`).Scan(&storedHash); err != nil {
		t.Fatal(err)
	}
	if storedHash != security.KeyedHash(server.Config.PIIHashKey, "919876500030") {
		t.Fatalf("phone was not linked: %q", storedHash)
	}

	// The same number must never move to a different customer.
	_, _ = db.DB.Exec(`INSERT INTO customers(id,shopify_id,whatsapp_consent,created_at,updated_at) VALUES(31,'gid://shopify/Customer/31','unknown',?,?)`, now, now)
	body = `{"records":[{"shopify_id":"gid://shopify/Customer/31","phone":"+919876500030"}]}`
	request = httptest.NewRequest(http.MethodPost, "/api/v1/customers/import", bytes.NewReader([]byte(body)))
	request.Header.Set("Authorization", "Bearer api")
	response = httptest.NewRecorder()
	server.Handler().ServeHTTP(response, request)
	var result map[string]any
	_ = json.Unmarshal(response.Body.Bytes(), &result)
	links, _ := result["phone_links"].(map[string]any)
	if links["conflict"] != float64(1) {
		t.Fatalf("a number moved between customers: %v", result)
	}
}
